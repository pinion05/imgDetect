...
function App(): JSX.Element {
  ...
  const [model, setModel] = useState<mobilenet.MobileNet | null>(null); // 추가된 부분
  ...
  function imgUpload(e: React.ChangeEvent<HTMLInputElement>) {
    if (e.target.files) {
      const uploadImg = e.target.files[0];
      const fileExtension = [...uploadImg.name.split(".")].pop();
      console.log(fileExtension);

      if (fileExtension === "png" || fileExtension === "jpg") {
        console.log(uploadImg);
        setImgURL(URL.createObjectURL(uploadImg));
      } else alert(`이미지가 아닙니다`);
    }
  }

  async function detectButonClick() {
    console.log(imgURL);
    if (model) {
      try {
        const result = await model.classify(imgRef.current);
        console.log(result);
        setResult(`${result[0].probability * 100}%  ${result[0].className}`);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  return (
    <>
      <Wrap dir="column">
        <h1>머신러닝 이미지 감지</h1>
        <PreviewImg ref={imgRef} src={imgURL} />
        <Spacing height={25} />
        <input type="file" onChange={imgUpload} />
        <Spacing height={25} />
        <button onClick={detectButonClick}>분석하기</button>
